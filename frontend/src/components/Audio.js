...
import { Waveform, DotPulse } from '@uiball/loaders';
...
export default function AudioRecorder() {
    ...
    const handleNextQues = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                // console.log("Chat is being initialised");
    ...
    const handleTry = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                handleFlashMessage("Audio not found. Please try again", false);
            } else if (response.status === 401) {
                handleFlashMessage("Unauthorized access. Please Login", false);
                setTimeout(() => {
                    window.location.href = '/';
        ...
        <div>
                ...
                {showGetFeedbackButton && sessionType === "practice" && !sttLoading && (
                    <div style={{ marginRight: '17px', marginLeft: '19px', marginTop: '8px' }}>
                        <center>
                            <button
                                onClick={handleTry}
                                disabled={uploading || sttLoading || recordingStatus === "recording" || endFlag === 0}
            ...
            {toggleState !== "Expand" && (
                <div>
                    ...
                    {audio && showAudio && !uploading && (
                        <div className="audio-container text-center">
                            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                {/* Display an audio element if audio is available */}
                    ...
                    {recordingStatus === "recording" && (
                        <center>
                            <br />
                            {/* Display a waveform when recording is active */}
                            <Waveform size={25} lineWeight={3.5} speed={1} color="purple" />
                        </center>
                    )}
                </div>
            )}
